...
func reviewerAssignment (prNumber, GITHUB_TOKEN string) error {
	author, err := membership.GetPullRequestAuthor(prNumber, GITHUB_TOKEN)
	if err != nil {
		return err
	}

	if membership.IsNoAssigneeUser(author) {
	...
	if requestedReviewer != ""{
		...
		if previousAssignedReviewers != nil {
			...
			for _, reviewer := range previousAssignedReviewers {
				if membership.IsTeamReviewer(reviewer) {
...
func getPullRequestRequestedReviewer(prNumber, GITHUB_TOKEN string) (string, error) {
	...
	_, err := membership.RequestCall(url, "GET", GITHUB_TOKEN, &requestedReviewers, nil)
	if err != nil{
		return "", err
	}
...
func getPullRequestPreviousAssignedReviewers(prNumber, GITHUB_TOKEN string) ([]string, error) {
	...
	_, err := membership.RequestCall(url, "GET", GITHUB_TOKEN, &reviews, nil)
	if err != nil{
		return nil, err
	}
...
func assignPullRequestReviewer(prNumber, assignee, GITHUB_TOKEN string) error {
	...
	reqStatusCode, err := membership.RequestCall(url, "POST", GITHUB_TOKEN, nil, body)
	if err != nil{
		return err
	}
...
func assignRandomReviewer(prNumber, GITHUB_TOKEN string) error {
	assignee := membership.GetRamdomReviewer()
