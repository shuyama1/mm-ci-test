...
func postBuildStatus(prNumber, GITHUB_TOKEN, commitSha string, body map[string]string) error {
	...
	url := fmt.Sprintf("https://api.github.com/repos/shuyama1/mm-ci-test/statuses/%s", commitSha)
...
func addAwaitingApprovalLabel(prNumber, GITHUB_TOKEN string) error {
	url := fmt.Sprintf("https://api.github.com/repos/shuyama1/mm-ci-test/issues/%s/labels", prNumber)
